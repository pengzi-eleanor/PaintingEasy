...
describe('search API mock boundary', () => {
  it('sends the unified JSON request and returns response', async () => { const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify(response), { status: 200 })); await expect(assistSearch('猫')).resolves.toEqual(response); expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/api/v1/search/assist'), expect.objectContaining({ method: 'POST', body: JSON.stringify({ query: '猫', language: 'auto' }) })) })
  it('sends the optional persona to the backend', async () => { const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify(response), { status: 200 })); await assistSearch('咖啡店', 'auto', 'graphic_designer'); expect(fetchMock.mock.calls[0][1]?.body).toBe(JSON.stringify({ query: '咖啡店', language: 'auto', persona: 'graphic_designer' })) })
