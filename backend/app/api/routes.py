...
from app.models import (
    ...
    ImageMetadata, ImageUploadResponse,
    ImageKeyword,
    SearchAssistRequest,
...
from app.providers import LocalImageStorageProvider, MockImageAnalyzeProvider
...
async def upload_image(file: Annotated[UploadFile, File()]) -> ImageUploadResponse:
    if file.content_type not in ALLOWED_IMAGE_TYPES:
        raise HTTPException(415, detail={"code": "UNSUPPORTED_IMAGE_TYPE", "message": "仅支持 jpg、jpeg、png、webp 格式图片"})
    ...
    if len(content) > settings.image_upload_max_size_mb * 1024 * 1024:
        raise HTTPException(413, detail={"code": "IMAGE_TOO_LARGE", "message": "图片大小不能超过 5MB"})
    try:
        with Image.open(BytesIO(content)) as parsed:
            parsed.verify()
            width, height, fmt = parsed.width, parsed.height, parsed.format
    except (UnidentifiedImageError, OSError):
        raise HTTPException(422, detail={"code": "INVALID_IMAGE", "message": "图片损坏或无法解析"})
    expected = {"JPEG": "image/jpeg", "PNG": "image/png", "WEBP": "image/webp"}
    if expected.get(fmt) != file.content_type:
        raise HTTPException(422, detail={"code": "INVALID_IMAGE", "message": "文件内容与图片类型不匹配"})
    image_id = f"img_{uuid4().hex}"
    now = datetime.now(timezone.utc)
    metadata = ImageMetadata(image_id=image_id, original_filename=file.filename or "image", stored_filename=f"{image_id}.{ALLOWED_IMAGE_TYPES[file.content_type]}", content_type=file.content_type, size=len(content), width=width, height=height, url=f"/uploads/{image_id}.{ALLOWED_IMAGE_TYPES[file.content_type]}", storage_path=str(Path(settings.image_upload_dir) / f"{image_id}.{ALLOWED_IMAGE_TYPES[file.content_type]}"), created_at=now, expires_at=now + timedelta(hours=settings.image_expire_hours))
    image_storage.save(content, metadata)
    return ImageUploadResponse(image_id=image_id, filename=metadata.original_filename, **metadata.model_dump(exclude={"image_id", "original_filename", "stored_filename", "storage_path", "status"}))


@router.post("/images/{image_id}/analyze", response_model=ImageAnalyzeResponse)
async def analyze_uploaded_image(image_id: str, platform: str | None = None, persona: str | None = None) -> ImageAnalyzeResponse:
    image = image_storage.get(image_id)
    if not image:
        raise HTTPException(404, detail={"code": "IMAGE_NOT_FOUND", "message": "图片不存在或已过期"})
    try:
        suggestions = image_analyzer.analyze(image, persona=persona, platform=platform)
    except Exception:
        suggestions = []
    candidates = [KeywordOptimizeService._candidate(item, index, image.original_filename) for index, item in enumerate(suggestions)]
    return ImageAnalyzeResponse(image_id=image_id, provider="mock", is_ai_generated=False, degraded=True, suggestions=candidates, messages=["当前使用 mock 图片语义识别，未调用真实 AI API"])
...
async def assist_search(payload: SearchAssistRequest) -> SearchAssistResponse:
    return KeywordOptimizeService().optimize(payload.query, payload.persona, payload.platforms, payload.language)
